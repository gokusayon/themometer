...
__NO_RETURN static void thrBUT(void *argument) {
  //uint32_t button_msk = (1U << Buttons_GetCount()) - 1U;
	...
	for (;;) {
		...
		// If change in state
	    if (state != last){
			// Toggle GPIO for right button. Set l_flag, then toggle LED.
	      	if(is_set(state, 7) == 1){
...
__NO_RETURN void app_main (void *argument) {
  ...
  (void)argument;
	...
	showIntegerDisplay(diff/25000000);
	printf("%lo", diff);
	//printf("Print %04X\r\n", diff);
	osDelay(900U);
	GUI_Clear();
	...
	while(1){
		...
		if(is_set(BSR, 3) != 0){
			...
			// T12 : Set BSR 
			// Set m_flag = 0. That is middle button click detected.
			BSR = clear_bit(BSR, 3);
		...
		// Point F.
		// If Right button pressed. check `F & 0000 0001`.
		if(is_set(BSR, 7) != 0){			
